...
(function () {
  ...
  // Expose goTo for carousel cross-page navigation
  window._scrollGoTo = goTo;
  ...
  window.addEventListener('touchend', e => {
    ...
    // If carousel is handling a horizontal swipe, skip page flip
    if (window._casHorizontal) return;
    const dy = ty - e.changedTouches[0].clientY;
    const dx = tx - e.changedTouches[0].clientX;
    const dt = Date.now() - ttime;

    // If the touch was a horizontal carousel swipe, skip page flip
    const track = document.getElementById('casesTrack');
    if (track && track.dataset.hswipe === '1') return;
